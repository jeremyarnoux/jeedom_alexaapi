...
function printEqLogic(_eqLogic) {
	...
	if (_eqLogic.configuration.refreshInterdit=='1') {
	$(".refreshAutorise").hide();
	$(".refreshInterdit").show();
	} else {
	$(".refreshAutorise").show();
	$(".refreshInterdit").hide();
	}
}

$('#bt_forcerRefresh').off('click').on('click', function () {
	//window.location.reload();
		$.ajax({
			type: "POST",
			url: "plugins/alexasmarthome/core/ajax/alexasmarthome.ajax.php",
			data: {
			  action: "forcerRefresh",
...
$('.eqLogicAttr[data-l1key=configuration][data-l2key=type]').on('change', function () {
    ...
    if (id) {
		console.log(id);
        jeedom.eqLogic.byId({ // Pour permettre d'afficher un ARRAY
            "id": $('.eqLogicAttr[data-l1key=id]')[0].value, noCache: true, success: function (obj) {
                ...
                if (obj && obj.configuration && obj.configuration.capabilitiesSmartHome && obj.configuration.capabilitiesSmartHome.length && obj.configuration.capabilitiesSmartHome instanceof Array) {
                    $('.eqLogicAttr[data-l1key=configuration][data-l2key=capabilitiesSmartHome]')[0].innerHTML = obj.configuration.capabilitiesSmartHome.join(', ');
                }            
			}
        });
    }
