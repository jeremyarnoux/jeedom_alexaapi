...
$("#bt_addalexaapiInfo").on('click', function(event) {
  var _cmd = {type: 'info'};
  addCmdToTable(_cmd);
});

$("#bt_addalexaapiAction").on('click', function(event) {
  var _cmd = {type: 'action'};
  addCmdToTable(_cmd);
});

$('#bt_healthalexaapi').on('click', function () {
  $('#md_modal').dialog({title: "{{Santé alexaapi}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexaapi&modal=health').dialog('open');
});

$('#bt_webalexaapi').on('click', function () {
  var nodeId = $('#idalexaapi').value();
  $('#md_modal').dialog({title: "{{Interface alexaapi}}"});
  $('#md_modal').load('index.php?v=d&plugin=alexaapi&modal=web&ip=' + nodeId).dialog('open');
});
...
$('body').on('alexaapi::includeDevice', function (_event,_options) {
  if (modifyWithoutSave) {
    ...
    if (_options == '') {
      ...
      window.location.href = 'index.php?v=d&p=alexaapi&m=alexaapi&id=' + _options;
    }
  }
});
